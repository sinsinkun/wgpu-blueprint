...
#[derive(Debug)]
pub struct App {
  pipelines: Vec<RPipelineIdV2>,
  textures: Vec<RTextureIdV2>,
  objects: Vec<RObjectIdV2>,
...
impl AppBase for App {
  fn init(&mut self, sys: SystemInfo, renderer: &mut RendererV2) {
  ...
  fn resize(&mut self, renderer: &mut RendererV2, width: u32, height: u32) {
    let h = height as f32;
    let w = h * 4.0 / 3.0;
    self.camera_overlay.target_size = Some(vec2f!(w, h));
    renderer.resize_texture(self.textures[0], Some(self.objects[0]), width, height);
  }
  fn update(&mut self, sys: SystemInfo, renderer: &mut RendererV2) -> Vec<RPipelineIdV2> {
    // process inputs
...
impl App {
  ...
  fn init_circle(&mut self, renderer: &mut RendererV2) {
    let pipe2 = renderer.add_pipeline(RPipelineSetupV2 {
      shader: RShader::FlatColor,
      ..Default::default()
    });
    let cir_data = Primitives::reg_polygon(10.0, 16, 0.0);
    let cir = renderer.add_object(RObjectSetupV2 {
      pipeline_id: pipe2,
  ...
  fn init_rounded_rect(&mut self, renderer: &mut RendererV2) {
    let pipe3 = renderer.add_pipeline(RPipelineSetupV2 {
